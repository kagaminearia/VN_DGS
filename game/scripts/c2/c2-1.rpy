label c2_1:
    "一缕光线穿过黑暗。"
    "而后，视野闪了闪，逐渐明亮起来。"
    by "……嗯？什么情况……"
    by "等等，总觉得这个场景好像似曾相识……靠。"
    me "嗯……你之前又晕倒了，现在还好吗？"
    by "身上好累……怀疑有人在我睡觉的时候偷偷揍我。"
    unknown "白一你醒了？"
    me "……还有其他人在？"
    "白一说话的声音立马止住，她眨了眨眼睛，装作什么都没发生，刚醒过来的样子。"
    ty "白一，你还好吧？现在都下午了，我还以为你要睡一天呢，吓到我了……"
    "前不久才讨论过的主角，天玉，出现在眼前，让白一愣了一下才反应过来。"
    by "所以是怎么回事？"
    ty "你在一千米测试的时候好像脚滑，跟前面的梁绵绵撞一起了。"
    ty "然后你们都摔倒在地上，你就晕过去了。"
    by "……撞，撞到人了？"
    ty "嗯，不过她没晕过去，就是腿擦伤了。你要是觉得不好，再买点药送给她就行。"
    by "噢……"
    ty "所以你呢，有没有哪里不舒服？还能动吗？"
    by "好像……还行。"
    "白一试着坐起来，动腿下床。"
    "脚触地的一瞬间，似乎有些提不起劲，好在她抓紧了单人床的床柱，免于再次摔倒。"
    ty "哇……小心！"
    "天玉赶紧扶住白一的肩膀。她满脸着急，看起来像被吓到了。"
    ty "你真的没关系吗？要不，我抱你吧？"
    by "抱——抱我？！"
    "白一声音提高，瞪大眼睛，表情比刚才的天玉还要惊恐。"
    ty "对呀，怎么了？"
    ty "之前也是我抱你到校医室的呀。"
    by "噗——咳咳……"
    me "哇，她这么厉害。"
    "天玉个子不算矮，比白一高一个头左右。"
    "但她的身形看起来是修长纤细的类型，难以想象是怎么把白一抱起来的。"
    "果然……该说不愧是人不可貌相吗……"
    by "不，不用了——我可以的。"
    "白一被吓得差点一个踉跄，但凭借惊人的求生欲稳稳站好。"
    by "那个，之前谢了。"
    ty "没事啊，我们是同学嘛，要互帮互助的。"
    by "……我没事了，你回去吧。"
    ty "啊……真的可以么？"
    by "当然。你才是应该回去上课吧。"
    ty "这个呀，不用担心，我跟老师说过了。"
    ty "我的话，空缺一两节课也没关系的。"
    "天玉温柔地笑着，那是一种类似“好学生的余裕”的自信吧。"
    by "……真没事。"
    ty "那你要注意休息哦，腿的位置容易再伤到。"
    ty "啊，对了——"
    by "……哈啊！"
    "打发走天玉后，白一一屁股坐回了床上，带着床垫重重地弹跳了一下。"
    by "草，痛死我了。"
    me "我还以为你的痛觉神经被撞失灵了呢……"
    "明明腿那么痛，却还能面不改色地站着说话，我都有点佩服了。"
    "如果不是我也会跟着痛的话就更好了……"
    by "那不然怎么办，难道真的让她来抱我走？"
    by "不行，光是说出来就感觉要吐了。呕。"
    me "……"
    "我想象了一下那个画面，确实有些……难以形容。"
    me "腿现在怎么样了？"
    "白一撩起裤腿，露出红肿的膝盖和擦伤的小腿。"
    by "其实外面不是很严重。"
    by "但……好像是因为我撞到那个人之后，她替我挡了一下。"
    by "完蛋了，我会不会被她的家长骂死。"
    me "那也……没办法……"
    "这事不管怎么说，她都不占理。"
    by "还好，至少我家不会有家长骂我，最多只会被骂一次。"
    me "……你还挺会安慰自己的。"
    by "嗯呢。"
    "虽然已经是下午，但还没到下课时间。"
    "校医室没有别人在，十分安静，白一干脆换了个舒适的姿势，靠坐在床上观察房间。"
    me "话说，老师去哪了？"
    by "可能提前下班了，或者有事走了？或者吃饭了？"
    by "随便啦，总归校医室也不实用的。"
    by "不弄乱东西，走的时候记得关门就好了。"
    "——地图"
    # 【单人床】套着简易被套，盖着薄被的铁架单人床。
    by "别说，睡起来还挺舒服的。"
    by "以后能不能翘课来这里睡觉啊。"
    me "你的想法很危险……"
    # 【房门】通往内侧房间的门，上面贴有几个可爱的贴纸。正中央挂着一张门牌，用圆润的字体画着“love”的字样。
    by "什么鬼？好恶心。"
    me "这是啥啊？"
    by "心理辅导室的门。"
    by "去年开始搞的，说什么要关注学生心理健康。"
    by "就是说你难过的时候可以来找老师聊聊天啊巴拉巴拉的。"
    me "嗯……这不好吗？"
    by "拜托，都是做做样子的好吧？"
    by "这房间都是临时装的，本来是校医室的杂物间来着。"
    by "就为了说出去面子上好看，还搞什么每个月的心灵讲座。"
    by "根本没人在听的好吗，听了也没什么用。"
    me "……"
    "总感觉这人怨气好重……我识相地闭嘴了。"
    # 【柜子】玻璃柜上了锁，可以看到里面摆放着创可贴，棉球，纱布，感冒冲剂等物品。
    me "好像没有温心吃的那种药，那个是药片我记得。"
    by "嗯，这边没有。"
    by "那个冲剂其实都没什么用，但学校里大概只能有这种东西。"
    me "说起来你喝过？"
    by "是啊，超级苦，关键是又苦又没用。"
    by "简直像不下雪的冬天一样耍流氓。"
    me "……"
    "铃——铃——"
    "下课铃声响起，结束了我跟白一在校医室内进行一圈的指指点点。"
    by "啊，下课了，回家。"
    me "你的腿还好吗，不用再缓一会？"
    by "没事啊，回家再休息一样的。"
    by "回家不积极，脑袋有问题~"
    me "……"
    "还是第一次听到白一用这种语气说话……颇有些惊悚的感觉。"
    "看得出来她是真的很想马上回家……"
    sw "白一！"
    "不速之客——对于现在的白一来说——闯入了校医室。"
    sw "啊太好了，你真的在这！"
    by "……你又是？"
    sw "我听说你受伤了，还很严重，都进校医室了，就想来看望你呀。"
    by "你的消息还真够灵通的。"
    sw "那当然！我可是——"
    by "停——打住。"
    "白一举起手臂，做了个中止的手势。"
    sw "欸~~"
    by "所以，看我干嘛？"
    sw "我们不是朋友了吗？当然要来关怀一下啊！"
    by "我们……是朋友吗？"
    sw "都交换过手机号码了，当然是啦。"
    sw "啊！你该不会要赖账吧？要对我始乱终弃了！"
    by "……"
    me "哈哈哈哈，她真有意思。"
    "难得看见白一吃瘪，我乐不可支地笑出了声。"
    by "闭嘴。"
    "白一不太想继续搭理这个话题。"
    "她揉了揉太阳穴，只觉得脑袋比腿还疼。"
    by "嗯……那你看完了。"
    by "哦对，有件事。"
    sw "嗯？怎么啦？"
    by "刚刚，天玉来找过我。"
    sw "噫——"
    "姒舞顿时拧起了眉毛，做出夸张的嫌恶表情。"
    sw "我知道，她抱着你来的！干嘛呀说这个。"
    by "呃……就是。"
    sw "完了，你是不是也觉得她人很好，很不错？"
    by "就……还行。"
    "白一实话实说。对她来说，不论是天玉还是姒舞，其实都没什么区别。"
    "姒舞好像受了很大打击，嘴巴一瘪，发出哭唧唧的声音。"
    sw "呜呜，我就知道……我就知道！"
    sw "为什么呀，为什么她总是比我还受欢迎呀……"
    by "……合着你不喜欢她就是嫉妒啊。"
    sw "才不是！！不喜欢就是不喜欢！第一眼看到就不喜欢！"
    by "好吧……随便。"
    "白一无意了解她和天玉的爱恨情仇，敷衍地摆了摆手。"
    by "我是想说，天玉说下周她生日，请班上的同学一起去玩。"
    by "她问我要不要去，然后也可以带熟悉的人去。"
    sw "啊？"
    sw "我好像前几天有听人说，她要请好多人。"
    sw "但你怎么也跟我说？"
    sw "难道你的意思是你也想去，还问我想不想跟着一起？"
    by "嗯。"
    sw "我刚刚才跟你骂完她欸！你竟然让我去捧她的场，呜呜——"
    by "……"
    by "免费蹭饭有什么不好的。"
    sw "呜呜——嗯？你说，你只是为了蹭饭啊……"
    by "对啊，不然呢。"
    "姒舞的装哭声戛然而止，她瞪着白一看了一会，突然恍然大悟般大叫一声。"
    sw "哦！！"
    sw "对啊，我懂了，我要去，让她看到我比她更受欢迎！"
    sw "真不错，你太棒了，好聪明呀，简直是天才！"
    sw "白一~~~"
    by "……"
    "面对姒舞的热情夸赞和沾沾自喜，白一表示拒绝。"
    by "我要回家休息了，你也早点走吧。"
    "放学后，教室里比以往还要吵闹。"
    "连白一这个不受欢迎的人，在进门时也没有再收到频频投来的目光。"
    by "……搞毛啊？"
    "好些人一起围在教室，天玉被围在中间，笑得很高兴。"
    by "哦……我知道了，是她在搞活动。"
    "学生们总是爱玩的，天玉请人吃饭玩乐，自然是点燃了大家的热情。"
    "就像春游一样，明明离日子还有好几天，也不能阻挡大家讨论怎么玩得开心。"
    "笑声一阵又一阵，话题从天南聊到海北。"
    # 【A】欸~还有这种事啊~
    # 【B】哈哈哈，开玩笑的啦。
    # 【A】不过，真的好难呐，离期末考太近了，最近睡觉都睡不好。
    # 【B】我也是，你看我的黑眼圈，呜呜~
    ty "我也有点……"
    # 【A】对呀对呀，所以能去天玉那玩一天太好了，放松一下！
    ty "唔，到时候我也送大家一点帮助睡眠的东西吧。"
    # 【B】还有这种东西呀？
    ty "嗯，助眠的，蒸汽眼罩，香薰，还有花茶包之类的。"
    ty "因为我平时也有这种情况嘛，所以在这方面还算比较有心得。毕竟失眠真的很不舒服呢。"
    # 【A】哇，好啊好啊，可以试试！
    by "……啧。"
    "跟教室中央的热闹相比，白一所在的位置显得格外安静。"
    "就仿佛彩色的校园里格格不入的一块白色一样。"
    "在闹哄哄的欢笑声里，她终于不动声色地收好书桌的东西，挎上书包，忙不迭地离开教室。"
    by "到时候——我去，好痛！"
    me "好痛！"
    by "我靠！你叫什么？吓我一跳。"
    "白一整个人一惊，差点再次踩空，为本就疼痛的伤口再添一笔。"
    me "……我也会感觉到痛啊。"
    by "哦，差点忘了这码事。"
    by "突然感觉好爽，不止我一个人疼。"
    by "哇塞，赞。"
    me "……"
    me "你这就是所谓的……伤敌八百，自损一千？"
    by "切，我乐意。"
    "白一撇了撇嘴，拖着腿一瘸一拐地慢慢走。"
    xs "听说你今天摔着了？"
    by "……是啊。"
    by "怎么你也来问一遍这个。"
    xs "关照一下你的身体健康啊。"
    by "我只是伤了，又不是残了，或者死了。"
    "白一翻了个白眼。"
    "尽管隔着手机，西顺看不见，但她似乎能想象到白一的反应，叹了口气。"
    xs "事实上，你有没有想过为什么。"
    xs "同样是摔了一跤，你的那个同学擦过药之后就去继续上课了。"
    xs "但你直接晕倒了，之后睡了一下午。"
    by "……什么意思。"
    xs "从比较科学的角度说，这是一种类似于免疫系统的疾病。"
    xs "你的身体受到影响，会导致身体各处虚弱无力，呼吸困难。"
    xs "从不那么科学的角度说……就是你身上的诅咒在消耗你的生命力。"
    "第一次说到白一身上的事情时，西顺就提到过诅咒的效果和三个月的期限。"
    "但资料的概念太过遥远，这是白一第一次切身体会到。"
    "体会到“自己也许就要死了”的事实。"
    by "哦……"
    by "好像是有这码事。"
    xs "当然我不是要让你有压力，只是想你对自己的情况有个具体的认知，也有个心理准备。"
    xs "其实，就像我一开始说的那样，不要有压力，但也最好注意一下身体。"
    by "好好，我明白。"
    "白一懒洋洋地开口。"
    by "就是说，万一你最后也没查出来，我也不会怪你的。"
    by "哦不不不，如果我真的死了好像也不能投诉你。"
    by "总之，真的没关系，哪怕你现在说你不想查了，也无所谓，我懂的。"
    xs "……我说你……"
    "西顺顿了顿，欲言又止，最后还是没有说完这句话。"
    xs "不，没事，你能保持这样的心态，也不错。"
    xs "不过，离那种山穷水尽的地步还远吧，不要说得那么悲观嘛。"
    by "……拜托，我是开玩笑的。"
    by "这，你都看不出来吗？"
    xs "是吗？"
    "西顺重重地拉长了声音，似乎是在透过电话里的语气了解白一的内心所想。"
    "隔着电话，白一放松了些，只是懒懒地靠在原地，并没有再多解释什么。"
    "但好在西顺也没再在这个话题上多说。"
    xs "那就好。"
    by "行了行了知道了。"
    by "刚好今天去校医室看了，是像你说的那样，没有温心吃的那个药。"
    xs "这么说来，也算是又有一个可切入的点。"
    xs "明面上看和温心交流比较多的人也就卫锋和姒舞，具体倒也不清楚了。"
    xs "可能还是你在学校多观察一下比较容易。"
    by "哦……尽量。"
    by "过几天我让姒舞跟我去同学生日会，算是按你说的，尝试打好关系吧。"
    "白一顿了顿，想到姒舞的热情，有些头疼。"
    xs "挺好。我还以为你不愿意呢。"
    by "……因为觉得她好像很好骗。"
    "西顺没忍住轻笑出声。"
    xs "有你这么说同学的吗？"
    xs "就算没问到什么，能做个朋友也是好的。"
    by "……哦。"
    "对此，白一显得有些兴致缺缺。"
    by "你刚刚怎么不说话？"
    "挂完电话，白一躺倒在床上，用手指敲了敲墙壁。"
    me "说什么话？"
    by "就跟平常一样啊，点评，感慨，骂我？"
    me "别加戏啊，我什么时候骂你了……"
    by "喂，这是重点吗？重点是我在问你。"
    me "……就是……感觉不太习惯？"
    by "哈？"
    me "西顺不是能听到嘛，就感觉，有点怪怪的。"
    me "类似于做坏事被发现的心情。"
    by "你也知道你骂我是坏事啊。"
    me "都说了我没有骂你啊——"
    by "好吧，我知道了知道了，就是能被她听到所以害怕呗？"
    me "嗯……差不多吧。"
    by "真是搞不懂，明明我也能听到你说话啊？"
    me "你当然是不一样的啊。"
    "这话听上去怪怪的……我刚说完就立马意识到不对，于是开始找补。"
    me "嗯我的意思是……毕竟我“借用”了你的身体。"
    me "西顺她，更像“外人”吧。"
    by "啧，你这么说，好像有道理。"
    by "明明被用的是我的身体，她竟然也能听见——"
    by "这样说的话好像是有些不爽。"
    me "不爽倒没有，不过……所以我不想那时候说话嘛。"
    by "好吧，准了。"
    "……这人突然就摆起了谱。"
    "我有些无语，但竟然又觉得理所应当。"
    "就好像……习惯了她就是会做这样的事。"
    "也习惯了她的存在。"
    return