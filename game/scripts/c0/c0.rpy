define pov = Character("[user]")
label c0:
    scene bg_black
    show black1 with w20
    "……"
    "好黑……"
    "什么东西……"
    scene bg_sickroom with vpunchm
    "——嗯？！"
    "短暂的眩晕后，眼前的场景变了。"
    "仍然很黑，但不是漆黑一片，而是能够在昏暗中看见隐约的轮廓，似乎是个拥挤的房间。"
    "房间没有开灯，看不清具体的边界，显得空荡荡的。"
    "这似乎是个完全陌生的地方。"
    "而我并不清楚发生了什么。"
    by eye_still o "什么，情况……"
    "……………………………………"
    "………………嗯……？"
    me "……还有人？"
    "——不是我自己的声音。"
    "房间里还有另一个人，让我顿时收紧了心神。"
    "有些沙哑的女声声音不大，却像惊雷一样在我的耳畔炸开。"
    by eye_shock o "谁在说话？！"
    scene bg_sickroom with vpunchm
    "视线猛地晃动一下，随后，变得明显的呼吸声传来，有些急促。"
    "她似乎比我更加慌张。"
    "但，这到底是什么情况……"
    by "有人吧？声音很近，人在哪？出来？"
    "视野开始摇晃，向左，向右，扫视一圈后又回到原地。"
    me "嗯……"
    "确认不是错觉后，我含糊地应了声，一时间不知道怎么继续接话。"
    by "有人是吗，为什么我看不见你？"
    "她的声音稍微提高一些，胸口的心脏也开始用力地快速跳动。"
    "不知道是错觉，还是黑暗和陌生的环境影响，憋闷感逐渐出现，令人难受。"
    "纠结好一会，我静静感受完，才斟酌开口。"
    me "你……我，有可能……我在你身体里？"
    by "什么？你在说什么鬼话？"
    "刷地一声，她猛地站起来，双腿似乎因为躺得太久，有些发软。"
    me "你不觉得我的声音在你脑子里吗？"
    "在这一点上，于我而言也差不多。她的声音就好像从我自己胸口发出，传到外界……"
    by "……"
    "她陷入沉默，没有说话，却忽然猛地抽出右手。"
    "手指紧紧地捏着一根笔杆，而后用力地划过左臂。"
    me "嘶……"
    "那不是笔，是小刀。鲜血一点点从皮肤上被割开的缝隙中渗出，伴随着尖锐的刺痛感。"
    me "你——"
    "刷。"
    "又是一下。她的动作很快，第二条血口子出现在左手臂上。"
    "第三条。"
    "第四条……"
    "……"
    me "……啊……"
    by "……还是听得到？是真的，不是做梦？不是幻觉？这科学吗？"
    by "我终于疯了？"
    "她捏着刀顿了顿，随后把手上的东西塞回口袋，小声地喃喃自语。"
    by "也没听过第二人格能跟主人格同时出现还对话啊……"
    me "不……你应该没疯，我应该也很清醒。"
    by "那你到底是什么东西？"
    me "……"
    "怎么还骂人呢？"
    "尽管这么说，我也很无奈。"
    "事实上，我什么都不记得，就这样出现在这里，看到她，或者说……跟在她身上。"
    by "刚刚你叫什么？"
    me "谁让你一言不合就动手啊……很痛的。"
    by "……你能感觉到痛？我的身体你也能感觉？"
    me "是啊。"
    by "那，你会跟我抢身体吗？"
    me "嗯……我只是能跟你说话，看到你看到的东西，还有感受……"
    by "你知道我是谁吗？你真的不是幻觉？不是我自己的妄想？"
    me "不，我……"
    "我的话刚说出口，又被她连珠炮似的语句打断。"
    by "所以真的存在？那你又是谁？你为什么在我脑子里，你是什么东西，什么新型病毒吗，致幻的？"
    me "……我不知道……"
    by "哈？？？"
    me "我不知道……别的我什么都不记得，听到你的声音的时候就在这里了。"
    by "哦……好吧。我知道了，你比我还没用。"
    me "……"
    "你才没用。"
    by "那你到底有什么是知道的。"
    me "你身上的事，难道不应该问你自己吗？"
    by "我，呃，我也不记得……不知道为什么。我之前还在家里睡觉，醒之后就在这里了。"
    "她的话一出，我们双双陷入沉默。"
    "无论从哪个方面说，这事都太过超出常理，让人没有任何头绪。"
    by "那怎么办，你不能滚吗？"
    me "……"
    "她说话还真是毫不留情……"
    me "应该不能……你就不能温柔一点吗？"
    by "面对你这种非要占着别人脑子的东西怎么温柔啊？"
    by "而且你真没用。你应该不会把我弄死吧？"
    me "……不会，准确地说，我什么都做不了。"
    "现在没有任何办法，也只能维持现状——她大概也是这么想，声音也不再那么暴躁。"
    by "好吧……那还好。你叫什么，怎么称呼？"
    python:
        user = renpy.input("想被称呼为？", length=15)
        user = user.strip()
        if not povname:
            user = "喂"
    if user == "喂":
        me "……"
        by "不想说就拉倒。无所谓。"
        by "啊，对，就叫你“喂”好了。"
        by "喂，怎么样？"
        me "……嗯。"
        "她自说自话很快，我只好顺从地应声。"
    else:
        me "你可以叫我[user]"
        by "哦，那，你好。"

    me "……你呢？"
    by "嗯……你可以叫我祖宗？"
    me "……"
    me "你是不是叫白一？"
    by "是……{size=45}嗯？？！！{/size}"
    by "你……你说什么？"
    me "我是不是说对了？"
    by "错了，你怎么可能知道我的名字，瞎说的吧。"
    me "真的吗？我觉得我没说错啊。"
    by "……好吧。你——说得没错……但你怎么会知道？"
    me "我能“看见”一些东西。"
    by "那是什么意思？"
    "咔哒。"
    me "这很难解释，我和你看到的东西也许不一样，就像是一个屏幕——"
    unknown "啊——！！！！"
    "巨大的尖叫声打断了我要说的话。"
    "白一转过头，而后四肢僵硬，愣在原地。"
    "不远处的门被打开——似乎是刚才对话时没注意其他声音——门后，灯光亮起，有些刺眼。"
    "光芒的最中间，黑洞洞的枪口对准了白一。"


    "——12月12日，星期日，城安局"
    by "什么情况……有毛病？这是在哪？要干嘛？"
    me "不知道。这里，好像是城安局。"
    by "哦……那应该是询问室了。"
    "白一坐在角落的凳子上，双手紧绷着握拳，放低了说话声音。"
    "刚才，经过短暂的对峙后，持枪的人扣住白一的手，把她拉到这个房间。"
    "房间里只有空空荡荡的书桌和两把凳子，让头上的顶灯更加晃人心神。"
    by "可能有人正在看我……我会不会被当成精神病？哦，我可能就是。"
    by "{size=20}待会有人进来，我不能这样跟你说话……你要是真的能“看见”什么，记得告诉我。{/size}"
    "她的声音越来越小，几乎只是在喉咙口吹气。除了能感受到她身体的我，大概没人能听清。"
    me "好。"
    xp "刚才的事很抱歉，是误会。只是我们有些问题想要问你。"
    by "噢……好。"
    "白一突然出现在这里，还失去意识，无论怎么想都十分奇怪，对方要询问也是理所当然。"
    "况且最重要的是，无论如何，在这个地方，白一并没有反抗的能力……"
    xp "前天下午和晚上的时候，你在哪里，做什么？前天，12月10日。"
    by "……12月10号？是……"
    "白一愣了愣，尽力含糊其辞——我知道，她是不记得之前的事情了。"
    me "今天是12月12号，星期日，我“看见”的。"
    "放在桌子下的右手悄悄移动，放在左臂上。白一用力按了按伤口，大概是表示听到了我的话。"
    by "嗯，周五，我……应该在家。"
    xp "是吗？有人看到你下午经过晟欧路路口。"
    by "那——可能是我记错了……"
    xp "这是前天发生的事，不是前年。"
    by "……"
    by "好吧……其实，我失忆了，不记得那天发生的事情。"
    "房间里陷入沉默，但意外的是，对面的人并没有在第一时间驳斥白一，而是拧起了眉毛，看着白一，似乎在等她继续说下去。"
    "没得到反应，白一吞了吞口水，只好轻轻地，缓缓地吐出一口气。"
    by "我只记得周四，我跟平时一样上学放学，晚上在家里睡觉。然后，醒来之后……就是刚才了。"
    xp "之前的事情全部还记得，只有这两天的事忘了？"
    by "嗯……对。"
    xp "那么，没有发生过其他事情吗？任何不确定的细节都可以。"
    by "……没有。"
    "短发女人没再说话，安静地坐在凳子上。"
    "正当白一开始觉得不耐烦，要开口询问时，门再次被打开，另一个长发女人走了进来。"
    xs "这位同学，知道“特调组”吗？"
    by "……啊？"
    "面对突如其来的问题，白一显得十分疑惑，但大概是学生的本能，还是老老实实地回答。"
    by "当然知道。特别案件调查组……是吧。{p}所以，是不是有什么危险的事情，跟我有关，所以你们才……？"
    xs "嗯，是，但不完全是。不过你那么想也没问题。"
    "西顺忽然露出一个浅笑，眼睛直勾勾盯着白一。"
    "深黑色的眸子在灯下隐约闪烁着反光，如同深邃的漩涡，摄取人的心神。"
    xs "其实特调组，还有另外一个名字。"
    xs "特殊怪异事件调查组。"
    by "……"
    "不知为何，也许是一种直觉，白一的呼吸短暂停止了一瞬间。"
    "但只是一瞬间。她很快又恢复气息，装作没有任何反应的样子。"
    by "……跟我说这个什么意思？"
    xs "嗯，这个啊，意思就是——"
    xs "你，或者说在你身上的“那个东西”，到底是什么？"
    
    return